...
const InputField = ({value, setValue, nameKey, type, invalidFields, setInvalidFields }) => {
  ...
  return (
    <div className="w-full relative">
       {value.trim() !== '' &&  <label className='text-[10px] animate-slide absolute top-0 left-[12px] block bg-white px-1'htmlFor={nameKey} >{nameKey?.slice(0,1).toUpperCase() + nameKey?.slice(1)} </label>}
        <input 
        type= {type || 'text' }
